...
servicePackageView(id:any){
  this.router.navigate(['/distributor/service/subService'],{
    queryParams:{id:id}
  });
}
    ...
    private _fetchData() {
      ...
      this.data=[];
      this.api.getAllGovernmentServices().subscribe(data=>{
        this.data = data.data;
        setTimeout(() => {
          if(!this.dataTable){
      ...
      if(this.create && !this.editS){
        ...
        this.api.createServices(data).subscribe(result => {
          if (result.status === 'error') {
            ...
            this.toast.success("Government Service Successfully Register");
            this.submitLoader = false;
            this.clearFilter();
            this._fetchData();
          }
    ...
    delete(id:any) {
      Swal.fire({
        title: 'You are about to delete a government service ?',
        text: 'Deleting your government service will remove all of your information from our database.',
        icon: 'warning',
        ...
        if (result.value) {
          this.api.deleteServices(id).subscribe(data=>{
            if (data.status === 'error') {
              ...
              Swal.fire('Deleted!', 'Government Service has been deleted.', 'success');
              this.submitLoader = false;
              this.clearFilter();
              this._fetchData();
            }
