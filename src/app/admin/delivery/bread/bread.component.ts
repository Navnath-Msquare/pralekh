...
export class BreadComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe((data: any) => {
      if (data.action == "create") {
        this.action = 1;
        this.title.setTitle("Create Delivery Partner | " + this.appC.title);
        this.label = 'Create Delivery Partner';
      } else if (data.action == "edit") {
        this.action = 2;
        this.title.setTitle("Edit Delivery Partner | " + this.appC.title);
        this.label = 'Edit Delivery Partner';
      } else if (data.action == "view") {
        this.action = 0;
        this.title.setTitle("View Delivery Partner | " + this.appC.title);
        this.label = 'View Delivery Partner';
      }
  ...
  submit() {
    ...
    if (this.action == 1) {
      ...
      this.apiS.createDelivery(data).subscribe(result => {
        if (result.status === 'error') {
          this.toastr.error(result.message);
          this.loader = false;
        } else {
          this.toastr.success("Delivery Parnter Successfully Register");
          this.loader = false;
          this.clearFilter();
